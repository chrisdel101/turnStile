defmodule TurnStile.StaffTest do
  ...
  describe "register_and_preload_employee/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Staff.register_and_preload_employee(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Staff.register_and_preload_employee(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Staff.register_and_preload_employee(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Staff.register_and_preload_employee(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Staff.register_and_preload_employee(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end
