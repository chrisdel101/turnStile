defmodule TurnStile.AdministrationTest do
  ...
  describe "register_admin/1" do
    ...
    test "validates email and password when given" do
      {:error, changeset} = Administration.register_admin(%{email: "not valid", password: "not valid"})
    ...
    test "registers admins with a hashed password" do
      ...
      {:ok, admin} = Administration.register_admin(valid_admin_attributes(email: email))
  ...
  describe "change_admin_registration/2" do
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Administration.change_admin_registration(
          %Admin{},
  ...
  describe "apply_admin_email/3" do
    ...
    test "applies the email without persisting it", %{admin: admin} do
      email = unique_admin_email()
      {:ok, admin} = Administration.apply_admin_email(admin, valid_admin_password(), %{email: email})
      assert admin.email == email
  ...
  describe "update_admin_email/2" do
    setup do
      ...
      token =
        extract_admin_token(fn url ->
          Administration.deliver_update_email_instructions(%{admin | email: email}, admin.email, url)
        end)
    ...
    test "does not update email if admin email changed", %{admin: admin, token: token} do
      assert Administration.update_admin_email(%{admin | email: "current@example.com"}, token) == :error
      assert Repo.get!(Admin, admin.id).email == admin.email
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end
  ...
  describe "change_admin_password/2" do
    ...
    test "allows fields to be set" do
      changeset =
        Administration.change_admin_password(%Admin{}, %{
  ...
  describe "reset_admin_password/2" do
    ...
    test "updates the password", %{admin: admin} do
      {:ok, updated_admin} = Administration.reset_admin_password(admin, %{password: "new valid password"})
