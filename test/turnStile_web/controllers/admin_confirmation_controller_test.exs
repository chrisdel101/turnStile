defmodule TurnStileWeb.AdminConfirmationControllerTest do
  ...
  describe "POST /admins/confirm" do
    ...
    test "sends a new confirmation token", %{conn: conn, admin: admin} do
      ...
      assert Repo.get_by!(Operations.AdminToken, admin_id: admin.id).context == "confirm"
    end

    test "does not send confirmation token if Admin is confirmed", %{conn: conn, admin: admin} do
      Repo.update!(Operations.Admin.confirm_changeset(admin))
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      refute Repo.get_by(Operations.AdminToken, admin_id: admin.id)
    end
