defmodule TurnStileWeb.AdminAuthTest do
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert Administration.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = Administration.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == "/"
      refute Administration.get_admin_by_session_token(admin_token)
    end
