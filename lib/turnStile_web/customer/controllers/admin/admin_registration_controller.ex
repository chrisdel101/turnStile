defmodule TurnStileWeb.EmployeeRegistrationController do
  ...
  def new(conn, _params) do
    changeset = Staff.change_admin_registration(%Employee{})
  ...
  def create(conn, %{"employee" => admin_params}) do
    ...
    if !current_admin do
      setup_initial_owner(conn, %{"employee" => admin_params})
    else
      organization_id = conn.path_params["id"]
      ...
      if length(organizations?) != 1 do
        ...
        current_user_permission do
          ...
          changeset = Staff.change_admin_registration(%Employee{}, admin_params)
  ...
  defp setup_initial_owner(conn, %{"employee" => admin_params}) do
    # confirm org exists
    organization_id = conn.path_params["id"]
    organizations? = TurnStile.Company.check_organization_exists_by_id(organization_id)
